...
export default function GapNoPdfExportRouteCodebaseImportsPdfLib() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-pdf-export-route-codebase-imports-pdf-lib', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>No PDF export route (codebase imports PDF lib but no route uses it)</h1>
      <p style={{ color: '#666', marginBottom: 16 }}>AIProposalSOWGenerator - Gap feature</p>
